...
import { Globe, Search, Sparkles, Users, Loader2 } from "lucide-react";
...
export interface FarmableCharacter {
  ...
  path: "Destruction" | "The Hunt" | "Erudition" | "Harmony" | "Nihility" | "Preservation" | "Abundance" | "Remembrance";
  rarity: number;
  talentBook: string;
  nation: string;
}

export interface FarmableLightCone {
  name: string;
  slug: string;
  type: string;
  rarity: number;
...
export interface DayRotationData {
  ...
  weapons: FarmableLightCone[];
}

const DAY_NAMES = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

const ELEMENT_COLORS: Record<string, string> = {
  Physical: "bg-[#9a9fa8]/15 text-[#ccd2dc] border-[#9a9fa8]/30",
  Fire: "bg-[#f45c43]/15 text-[#ff8069] border-[#f45c43]/30",
  Ice: "bg-[#54b4f5]/15 text-[#80cbff] border-[#54b4f5]/30",
  Lightning: "bg-[#b86df5]/15 text-[#d499ff] border-[#b86df5]/30",
  Wind: "bg-[#45d9a0]/15 text-[#73f0c0] border-[#45d9a0]/30",
  Quantum: "bg-[#646ef5]/15 text-[#9199ff] border-[#646ef5]/30",
  Imaginary: "bg-[#f5a623]/15 text-[#ffbe4a] border-[#f5a623]/30",
};
  ...
  return (
    <>
      <section className="page-heading">
        <div>
          <h1>Trailblaze Command</h1>
          <p>Find what characters and light cones you can farm today across the cosmos.</p>
        </div>
            ...
            {(["asia", "america", "europe"] as ServerRegion[]).map((region) => (
              <button
                ...
                className={`px-2 py-0.5 rounded text-[11px] font-mono transition-all cursor-pointer ${
                  server === region
                    ? "bg-[var(--accent)] text-[#1c1304] font-bold shadow-sm"
      ...
      <section className="rotation-section" id="rotation" aria-labelledby="rotation-title">
        ...
        <div className="section-header flex-wrap gap-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p>
                {SERVER_CONFIG[server].label} · {selectedDay === 0 ? "Sunday Universal Access: All Calyxes open" : "Crimson Calyx & Stagnant Shadows open"}
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-xs font-mono transition-all cursor-pointer flex items-center gap-1 ${
                    isSelected
                      ? "bg-[var(--accent)] text-[#1c1304] font-bold shadow-[0_0_10px_rgba(245,166,35,0.4)]"
                      : isServerToday
                      ? "bg-white/10 text-[var(--green-2)] font-semibold"
                      : "text-[var(--text-muted)] hover:text-[var(--text-light)]"
                  }`}
                >
                  <span>{dayLabel}</span>
                  {isServerToday && (
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${
                        isSelected ? "bg-[#1c1304]" : "bg-[var(--green-2)]"
                      }`}
                    />
                  )}
                </button>
        ...
        <div className="p-3.5 border-b border-white/5 bg-[var(--surface-raised)]/60 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all flex items-center gap-2 cursor-pointer ${
                activeTab === "characters"
                  ? "bg-[#f5a623] text-[#1c1304] font-bold shadow-[0_0_14px_rgba(245,166,35,0.4)]"
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all flex items-center gap-2 cursor-pointer ${
                activeTab === "weapons"
                  ? "bg-[#ffbe4a] text-[#1c1304] font-bold shadow-[0_0_14px_rgba(255,190,74,0.4)]"
                  : "bg-[var(--surface-sunken)] text-[var(--text-muted)] hover:text-[var(--text-light)] border border-white/5"
              }`}
            >
              <Sparkles size={14} />
              <span>Light Cones ({currentData.weapons.length})</span>
            </button>
          </div>
          ...
          <div className="relative min-w-[220px] max-w-xs flex-1">
            ...
            <input
              type="text"
              placeholder={activeTab === "characters" ? "Search character, path, element..." : "Search light cone, path, material..."}
              value={searchQuery}
        ...
        {loading && (
          <div className="p-12 flex flex-col items-center justify-center gap-2 text-xs text-[var(--text-muted)]">
            ...
            <span>Loading Trailblaze schedule...</span>
          </div>
        )}
          ...
          <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {filteredCharacters.map((char) => {
              ...
              return (
                ...
                >
                  <div className="w-16 h-16 relative rounded-lg overflow-hidden bg-[var(--surface-raised)] flex items-center justify-center text-lg font-bold text-[var(--green-2)]">
                    {char.name.slice(0, 2)}
                  </div>
                  ...
                  <div className="text-center w-full min-w-0">
                    ...
                    <span className="text-[10px] text-[#ffbe4a] block mt-1 font-medium truncate">
                      {char.talentBook}
                    </span>
                  </div>
                </Link>
        ...
        {/* Content Area: Direct Light Cone Cards */}
        {!loading && activeTab === "weapons" && (
          <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {filteredWeapons.map((weapon) => {
              return (
                <Link
                  href={`/database/light-cones/${weapon.slug}/`}
                  key={weapon.slug}
                  className="bg-[var(--surface-sunken)] border border-white/5 hover:border-[#f5a623] rounded-xl p-2.5 flex flex-col items-center gap-2 transition-all hover:scale-[1.02] group"
                >
                  <div className="w-16 h-16 relative rounded-lg overflow-hidden bg-[var(--surface-raised)] flex items-center justify-center text-lg font-bold text-[#f5a623]">
                    LC
                  </div>

                  <div className="text-center w-full min-w-0">
                    <div className="flex items-center justify-center gap-1">
                      <strong className="text-xs font-semibold text-[var(--text-light)] group-hover:text-[#f5a623] truncate" title={weapon.name}>
                        {weapon.name}
                      </strong>
                    </div>

                    <div className="flex items-center justify-center gap-1 mt-1">
                      <span className="text-[10px] px-1.5 py-0.2 rounded border font-mono bg-[#f5a623]/15 text-[#ffbe4a] border-[#f5a623]/30">
                        {weapon.type}
                      </span>
                      <span className="text-[10px] text-[var(--gold)] font-mono">
                    ...
                    <span className="text-[10px] text-[#f5a623] block mt-1 font-medium truncate" title={weapon.material}>
                      {weapon.material}
                    </span>
                  </div>
                </Link>
            ...
            {filteredWeapons.length === 0 && (
              <div className="col-span-full p-8 text-center text-xs text-[var(--text-muted)]">
                No farmable light cones found matching &quot;{searchQuery}&quot;.
              </div>
            )}
          </div>
        )}
